...
describe("stringShift test", () => {
  it("shifts correctly", () => {
    expect(stringShift("Hello", 3)).toBe("Khoor");
    expect(stringShift("~Z4", 3)).toBe('"]7');
  });

  it("return the original string for negative shift", () => {
    expect(stringShift("Hello", -1)).toBe("Hello");
  });

  it("returns the original string for non-numeric shift", () => {
    expect(stringShift("Hello", "abc")).toBe("Hello");
  });

  it("check circular shift", () => {
    expect(stringShift("~", 1)).toBe(" ");
  });

  it("check NAN",() =>{
    expect (stringShift("Hello", NaN)).toBe("Hello");
  })
});


describe('stringUnshift test', () => {
  it('unshifts  correctly ', () => {
      expect(stringUnshift("Khoor", 3)).toBe("Hello");
      expect(stringUnshift("]7", 3)).toBe("Z4");
  });

  it('returns the original string for negative unshift', () => {
      expect(stringUnshift("Khoor", -1)).toBe("Khoor");
  });

  it('returns the original string for non-numeric unshift', () => {
      expect(stringUnshift("Khoor", "abc")).toBe("Khoor");
  });

  it('check circular unshift', () => {
      expect(stringUnshift(" ", 1)).toBe("~"); 
  });
